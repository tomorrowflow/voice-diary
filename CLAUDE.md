...
Both the iOS app and the server's HTMX UI consume one set of tokens, generated by Style Dictionary from `docs/design-system/tokens/`.

- **Source of truth**: `docs/design-system/tokens/{color,layout,typography}/*.json` + `docs/design-system/specs/components/*.json`.
- **Build**: `bash scripts/build_design_system.sh` rebuilds the tokens and copies the generated outputs into both consumer trees. Run after any token edit and commit the regenerated files.
- **iOS outputs** (auto-generated, do not edit): `ios/Sources/DesignSystem/{DSColor,DSMetrics,DSSemantic}.swift`. Hand-written seam: `Theme.swift` (use `Theme.color.text.primary`, `Theme.spacing.md`, `Theme.radius.lg`, `Theme.font.body`, `Theme.motion.snappy`) + `DSButtonStyle.swift` (`.dsPrimary()`, `.dsSecondary()`, `.dsOutline()`, `.dsGhost()`, `.dsDestructive()` × `.sm`/`.md`/`.lg`).
- **Server outputs** (auto-generated): `server/webapp/static/{tokens.css,tokens-semantic.css}`. Hand-written: `static/fonts.css` (`@font-face` for Geist) and `static/style.css` whose top-level `:root` is a thin alias layer mapping legacy names to the DS variables.
- **Fonts**: Geist + Geist Mono variable TTFs (SIL OFL 1.1, see `Resources/Fonts/OFL.txt`) bundled in both apps. iOS via `UIAppFonts`. Server via `@font-face`.
- **Light/dark**: iOS uses `UITraitCollection`-driven dynamic `Color`s (auto-generated in `DSSemantic.swift`); server uses `prefers-color-scheme`. No code path manually toggles a theme.

**Hard rules.**
1. **Never hard-code colours, spacing, radius, or font sizes in feature code.** Always go through `Theme.*` (Swift) or `var(--*)` (CSS). The token output files are the only place literal values live.
2. **Don't edit `DSColor.swift`, `DSMetrics.swift`, `DSSemantic.swift`, `tokens.css`, or `tokens-semantic.css` by hand.** Edit the source JSON in `docs/design-system/tokens/` and rerun the build script.
3. **New components register themselves at `Theme.*`** (Swift) or via dedicated CSS classes that consume the alias layer — feature code should not reach into `DSColor`/`DSMetrics` directly.
4. **Buttons go through `DSButtonStyle`.** The five variants + three sizes match `specs/components/button.json`; if a button doesn't fit any of them, propose extending the spec rather than ad-hoc styling.
...
9. **Design tokens are the only source of style.** No hard-coded colours, spacing, radius, or font sizes in feature code. iOS goes through `Theme.*` and `DSButtonStyle`; server CSS goes through `var(--*)`. Token JSON edits trigger a rebuild via `scripts/build_design_system.sh`.
