...
NB: an earlier slice (07a–07d) extended the catalog with filesystem-
backed custom + LibriVox voices to support cloning. That capability
turned out to be impossible on the open-source Voxtral checkpoint —
Mistral withheld the audio encoder needed to extract embeddings from
reference audio (see docs/prd/voxtral-tts-integration.md). The slice
was reverted; the catalog now ships in its pre-07a shape.
